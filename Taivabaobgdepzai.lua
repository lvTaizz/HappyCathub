...
local Tabs = {
	...
	C = Window:AddTab({
		Title = "Tab pvp-Combat",
		Icon = ""
	}),
	tele = Window:AddTab({
		Title = "Tab Tween Island",
		Icon = ""
	}),
	an = Window:AddTab({
		Title = "TabMisc Sever",
...
local MaxFire = 1000
local FireCooldown = 0.6
local FireL = 0

CancelCoolDown = LPH_JIT_MAX(function()
	local ac = CombatFrameworkR.activeController
	if ac and ac.equipped then
		AttackCoolDown = tick() + (FireCooldown or 0.988) + ((FireL/MaxFire)*0.2)
		RigEven.FireServer(RigEven,"weaponChange",ac.currentWeaponModel.Name)
		FireL = FireL + 1
		fask.delay((FireCooldown or 0.988) + ((FireL+0.4/MaxFire)*0.1),function()
			FireL = FireL - 1
		end)
	end
end)
...
AttackFunction = LPH_JIT_MAX(function(typef)
	...
	if ac and ac.equipped then
		...
		if #bladehit > 0 then
			...
			if tick() - cooldowntickFire > 5 then
...
DamageAura = LPH_JIT_MAX(function()
	spawn(function()
		while game:GetService("RunService").Stepped:Wait(9) do
            pcall(function()
                local ac = CombatFrameworkR.activeController
...
local Toggle = Tabs.Race:AddToggle("Auto Complete Trial", {
	Title = "Auto buy gear to trial + train",
	Default = false
})
